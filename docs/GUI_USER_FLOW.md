...
v0.5 Live Capture는 현재 실행 흐름이 아니라 read-only 준비 확인 흐름입니다.

```text
open dashboard
-> open /live-capture
-> confirm domain allowlist requirement
-> confirm capture start/stop is a separate PR
...
| `/live-capture` | v0.5 live capture 구현 전 준비 조건과 보안 경계를 확인 | 조회 전용. capture start/stop, collector/receiver 변경, POST action, 실행 버튼 없음 |
...
| `/safe-files?project=<alias>` | safe files 4개의 존재 여부와 fingerprint 확인 | 조회 전용 safe file inventory checklist. 파일 본문 preview, download, form, POST action 없음 |
...
2. `Live Capture 준비`: `/live-capture`에서 domain allowlist와 read-only 준비 경계를 확인합니다. capture start/stop은 별도 PR입니다.
...
6. `Finding triage index`: 후보 metadata와 수동 검토 경계를 확인합니다.
7. `Report`: `report_draft.md`를 작성하거나 갱신합니다.
8. `Report readiness index`: 보고서 초안 metadata와 수동 검토 항목을 확인합니다.
9. `Workflow status index`: 조회 전용 workflow 체크리스트를 확인합니다.
10. `AI-safe preflight`: 조회 전용 핸드오프 사전 점검을 확인합니다.
11. `AI handoff index`: 안전 파일 4개와 권장 순서를 확인합니다.
12. `Prompt readiness index`: prompt 파일 상태와 수동 검토 경계를 확인합니다.
13. `Evidence boundary index`: 정제 evidence와 raw 금지 범위 경계를 확인합니다.
14. `Operator runbook index`: 수집부터 AI 투입 전 수동 검토까지 운영 순서를 다시 확인합니다.
15. `Safe file inventory index`: safe files 4개의 존재 여부, 크기, 수정 시각, SHA-256 fingerprint를 확인합니다.
16. `Export`: 안전 파일 4개만 dashboard export directory로 복사합니다.
...
`Live Capture 준비`는 조회 전용 GET page이며 capture start/stop, collector/receiver
동작 변경, ChatGPT 자동 전송, POST action을 포함하지 않습니다. live capture
설계 경계는
...
`Simple Dashboard`도 조회 전용 GET page이며 현재 상태와 다음 행동 링크만 제공합니다.
